    ...
    public class SQLWalkRepository : IWalkRepository
    {
            ...
            string? sortBy = null, bool isAcsending = true, int pageNumber = 1, int pageSize = 1000)
        {
            ...
            if (string.IsNullOrWhiteSpace(sortBy) == false)
            {
                ...
                {
                    walk = isAcsending ? walk.OrderBy(x => x.Name) : walk.OrderByDescending(x => x.Name);
                }
                else if(sortBy.Equals("LengthInKm", StringComparison.OrdinalIgnoreCase))
                {
                    walk = isAcsending ? walk.OrderBy(x => x.LengthInKm) : walk.OrderByDescending(x => x.LengthInKm);
                }
            }
